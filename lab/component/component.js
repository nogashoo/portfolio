/* =====================================================================
   Lab / 層 3「部品」のページ（/lab/component/）

   部品は「状態を持つもの」なので、見本は 1 個ではなく**並びと状態を一枚に
   並べた見本帳**。ボタンなら主・副・押せない・大中小をまとめて出す。

   見本は本物として動く（押す・書く・開く）。だからサムネイルもプレビューも
   **等身大に近い幅（720px）で描く**。場面や骨格のように 1180px で描いて
   3 割に縮めると、ボタンが 12px になって読めない。

   見本の組み立て（下地 → 動き → 橋 → 部品の連結）と採寸は lab-core.js が持つ。

   読み込み順は lab-core.js → data/{layers,foundation,motions,components}.js
   → component.js（いずれも defer）。
   ===================================================================== */
(() => {
    "use strict";
    ...
    const parts = window.LAB_COMPONENTS || [];
    const foundations = window.LAB_FOUNDATIONS || [];
    let kinds = window.LAB_COMPONENT_KINDS || [];
    if (!parts.length || !foundations.length) return;
    ...
    normalize(parts, "部品");
    if (window.LAB_LAYERS) window.LAB_LAYERS.check("component", parts.length);

    kinds = kinds.filter((k) => parts.some((p) => p.kind === k.id));
    const byId = new Map(parts.map((p) => [p.id, p]));

    /* 部品は等身大で見せる。1180px で描いて 3 割に縮めると、
       ボタンの文字が 4px になって「状態の違い」が読めなくなる */
    const WIDTHS = { pc: 720, sp: 390 };
    ...
    const fitDetail = () => {
        ...
        if (badge && r) {
            badge.textContent = `${w}px · ${Math.round(r.scale * 100)}% — そのまま触れます`;
        }
    };
